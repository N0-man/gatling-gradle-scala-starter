...
class TrafficLoadConfiguration(
  ...
  val incrementUsersPerSec = atLeast1(atMost20(totalNumberOfUsers / 5))
  val repeatLevels = atLeast1(
    (totalNumberOfUsers / incrementUsersPerSec) - 1
  )

  val eachLevelLasting = atLeast1Minute(desiredCycleDuration)
  val rampDuration = atLeast10Seconds(desiredRampDuration)
  val startingRate = atLeast1(initialNumberOfUsers)
